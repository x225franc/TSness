...
router.post("/", async (req, res) => {
	try {
		...
		if (Array.isArray(sharedWith) && sharedWith.length > 0) {
			...
			if (badgeCommunity) {
				const hasCommunityBadge = user.badges.some(
					(b: any) => b.name === badgeCommunity.name
				);
	...
	try {
		...
		if (unapprovedGyms.length > 0) {
			return res.status(400).json({
				erreur: `Impossible de créer un défi pour des salles non approuvées: ${unapprovedGyms
					.map((g) => g.name)
					.join(", ")}`,
			});
		}
...
router.put("/owner/:challengeId", async (req, res) => {
	...
	try {
		...
		if (ownerGyms.length !== challenge.gymIds.length) {
			return res
				.status(403)
				.json({ erreur: "Vous n'êtes pas autorisé à modifier ce défi" });
		}
		...
		if (unapprovedGyms.length > 0) {
			return res.status(400).json({
				erreur: `Impossible d'associer des salles non approuvées: ${unapprovedGyms
					.map((g) => g.name)
					.join(", ")}`,
			});
		}
...
router.delete("/owner/:challengeId", async (req, res) => {
	try {
		...
		if (ownerGyms.length !== challenge.gymIds.length) {
			return res
				.status(403)
				.json({ erreur: "Vous n'êtes pas autorisé à supprimer ce défi" });
		}
...
router.post("/complete/:challengeId", async (req, res) => {
	try {
		...
		if (
			...
			user.challenges_completed.includes(challenge._id)
		) {
