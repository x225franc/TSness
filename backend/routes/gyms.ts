...
router.put("/:id", async (req, res) => {
	try {
		const gym = await GymModel.findByIdAndUpdate(req.params.id, req.body, {
			new: true,
		});
...
router.patch("/:id/assign", async (req, res) => {
	try {
		const update = req.body.$unset
			...
			: {
					...(req.body.hasOwnProperty("exerciseTypeId") && {
...
router.get("/owner", async (req, res) => {
	try {
		...
		if (!ownerId)
			return res.status(400).json({ erreur: "ID propriétaire manquant" });
	...
	try {
		...
		if (!gym)
			return res.status(404).json({
				erreur: "Aucune salle trouvée pour ce propriétaire avec cet ID",
			});
		res.json(gym);
	} catch (err) {
		res.status(400).json({ erreur: (err as Error).message });
	...
	try {
		...
		if (existingGymsCount >= 4) {
			return res.status(400).json({
				erreur:
					"Limite atteinte : un propriétaire ne peut pas avoir plus de 4 salles de sport",
			});
		}
