...
// À lancer avec : npx ts-node backend/scripts/recalculate_scores.ts
  ...
  for (const user of users) {
    ...
    if (Array.isArray(user.challenges_completed)) {
      for (const challengeId of user.challenges_completed) {
        ...
        if (Array.isArray(challenge.sharedWith) && challenge.sharedWith.map(String).includes(String(user._id))) {
          score += 10;
        }
      }
    }
