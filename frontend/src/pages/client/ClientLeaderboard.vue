<script setup>
	...
	const loadLeaderboard = async () => {
		const res = await fetch(
			window.config.BACKEND_URL + "/api/user/leaderboard"
		);
		if (res.ok) {
...
<template>
	<div
		class="min-h-screen bg-gradient-to-br from-green-100 via-white to-green-200 py-10"
	>
		<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
			<h1
				class="text-4xl font-extrabold text-green-700 mb-8 text-center flex items-center justify-center gap-3"
			>
				<svg
			...
			<div
				class="bg-white rounded-2xl shadow-lg border border-green-200 overflow-hidden"
			>
				<table class="w-full text-left">
					...
					<tbody>
						<tr
							...
							:class="[
								i === 0
									? 'bg-yellow-100 font-extrabold text-yellow-700'
									: i === 1
						...
						>
							<td class="px-4 py-2">
								<span v-if="i === 0">
									<svg
										class="w-6 h-6 inline text-yellow-400 mr-1 align-middle"
										fill="currentColor"
										viewBox="0 0 24 24"
									>
										<path
							...
							<td class="px-4 py-2 text-green-800 font-bold">
								{{ user.score }}
							</td>
						</tr>
					</tbody>
